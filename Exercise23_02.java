...
public class Exercise23_02 {
    ...
    private static <E extends Comparable<E>> void merge(E[] list1, E[] list2, E[] temp) {
        int c1 = 0;   // pointer into list1
        int c2 = 0;   // pointer into list2
        int c3 = 0;   // pointer into temp (result)
    ...
    //
    // Same structure as to-do 2, but use:
    ...
    private static <E> void merge(E[] list1, E[] list2, E[] temp, Comparator<? super E> comparator) {
        ...
        while (c1 < list1.length && c2 < list2.length) {
            if (comparator.compare(list1[c1], list2[c2]) < 0) {
                temp[c3++] = list1[c1++];
            } else {
                temp[c3++] = list2[c2++];
            }
